...
* **🔄 挂载监控**：自动检测 CD2 挂载状态，当挂载丢失时自动重启容器，无需手动干预。
...
| `MOUNT_CHECK_INTERVAL` | `30` | 挂载检测间隔（秒） |
| `MOUNT_CHECK_ENABLED` | `true` | 是否启用挂载监控（设为 `false` 可关闭） |

> **挂载监控说明**：当 CD2 重启或更新导致挂载丢失时，容器会自动检测并重启，配合 `restart: unless-stopped` 策略实现自动恢复。
...
**Q: CD2 重启后容器会自动恢复吗？**
A: **会。** 容器内置挂载监控功能，每 30 秒检测一次挂载状态。当检测到 CD2 挂载丢失时，容器会自动退出并由 Docker 重启策略自动恢复。你可以通过设置 `MOUNT_CHECK_INTERVAL` 环境变量调整检测间隔。
